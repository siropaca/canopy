...
`running` は**走っている操作の列**から真偽値に落とす。  
一括フェッチとユーザーの操作が重なったとき、真偽値だけで持つと先に終わった方が実行中の表示を消してしまう。  
列の中身はステータスバーの文言に使う ([../adr/0023-progress-in-the-status-bar.md](../adr/0023-progress-in-the-status-bar.md))。  
終わったときは**その操作を名指しで抜く。** 末尾を抜くと、種別の違う 2 本が重なったときに残る側がずれる。

取り直しの最中は `reading` に別で持つ。  
`.git` の変化のたびに走るので、`running` に混ぜるとボタンが頻繁に無効になる ([../adr/0022-auto-refresh.md](../adr/0022-auto-refresh.md))。  
入るのは「更新」の 3 つの引き金から走った読み取りだけで、起動時の全件読み込みは入らない。
...
| フィールド | 型 | 意味 |
| --- | --- | --- |
| `dir` | `string` | git を実行したディレクトリ。**登録したパスとは限らない** (別のワークツリーで走ることがある) |
...
## repos_changed のペイロード

`.git` が変わったリポジトリの id の配列 ([../adr/0022-auto-refresh.md](../adr/0022-auto-refresh.md))。  
型は `string[]` だけなので専用の struct を持たない。

**1 件ずつではなくまとめて送る。** git の 1 操作で `.git` の中の何十ファイルも動くので、Rust 側で畳んでいる。  
フロントはこれを受けて、実行中でないリポジトリのスナップショットを取り直す。
...
**項目を足すだけなら設定ファイルの版 (`version`) は上げない。**  
足りない項目は既定値で埋める (`serde(default)`)。  
版を上げるのは、既にある項目の意味や形が変わって**変換が要るとき**だけ。  
足しただけで版を上げると、前の版のアプリが「新しすぎる」と判定して読めなくなる ([../adr/0016-store-without-plugin.md](../adr/0016-store-without-plugin.md))。  
読み込んだ設定は、その場で今の版に揃えてから書き戻す。
