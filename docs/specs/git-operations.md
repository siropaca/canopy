...
**導線を出すのは git を実行した失敗だけ。** コピーのように git を走らせない失敗は出力が無いので、文言だけを出す ([ui.md](ui.md) の「トースト」)。
...
**打ち切ったときも、それまでに出た stdout / stderr は残す。**  
締め切りが掛かっている間もパイプを読み続けているため ([../adr/0020-process-group-kill.md](../adr/0020-process-group-kill.md))。  
出力が 1 行も無いまま打ち切られることもあるので、コマンド行と文言は必ず残す。

打ち切った実行は**終了コードを持たない**。  
撃ったあとに読める値はこちらが送ったシグナルの結果なので、成否として使わない。
